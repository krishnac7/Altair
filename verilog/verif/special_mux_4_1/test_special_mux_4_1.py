...
@cocotb.test()
async def test_special_mux_4_1(dut):
    ...
    for i in range(16):
        ...
        if sel_0+sel_1+sel_2+sel_3<=1:
            assert dut.o_error_selection.value.binstr == "0", "Bad error detection"
            expected_en = 0
            expected = []
            if sel_0:
                expected = orig_array[3]
                sel_0 = 0
                expected_en = 1
            elif sel_1:
                expected = orig_array[2]
                sel_1 = 0
                expected_en = 1
            elif sel_2:
                expected = orig_array[1]
                sel_2 = 0
                expected_en = 1
            elif sel_3:
                expected = orig_array[0]
                sel_3 = 0
                expected_en = 1
            else:
                expected = orig_array[0]
            ...
            print("Expected: "+bin(tm))
            assert dut.o_outputs.value == expected, "Bad output value"
        else:
            print("i>1")
